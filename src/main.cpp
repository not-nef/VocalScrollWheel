...
class VolumeControlOverlay : public CCNode {
    protected:
        ...
        bool init(const int tag) {
            ...
            m_pSliderSFX->setValue(
                FMODAudioEngine::sharedEngine()->m_effectsVolume
            );
    ...
    public:
        ...
        void updateValue(float offset, bool sfx) {
            ...
            auto ovol = sfx ? fe->m_effectsVolume : fe->m_backgroundMusicVolume;
            ...
            if (sfx) {
                fe->m_effectsVolume = vol;
                fe->m_currentSoundChannel->setVolume(vol);
            } else {
            ...
            m_pPercentMusic->setString(
                (formatToString(round(fe->m_backgroundMusicVolume * 100.0f), 0u) + "%").c_str()
            );
            m_pPercentMusic->limitLabelWidth(20.0f, .35f, .2f);
            m_pPercentSFX->setString(
                (formatToString(round(fe->m_effectsVolume * 100.0f), 0u) + "%").c_str()
            );
...
class $modify(CCMouseDispatcher) {
    bool dispatchScrollMSG(float deltaY, float param_2) {
        if (!CCMouseDispatcher::dispatchScrollMSG(deltaY, param_2)) {
            return false;
        }
        ...
        if (kb->getAltKeyPressed()) {
            ...
            return CCMouseDispatcher::dispatchScrollMSG(deltaY, param_2);
    }
};